...
var CmdClient = &cobra.Command{
	...
	Long:  "Generate the proto client code. Example: xt proto client helloworld.proto",
	Run:   run,
}
...
func init() {
	if protoPath = os.Getenv("XT_PROTO_PATH"); protoPath == "" {
...
func run(_ *cobra.Command, args []string) {
	...
	if err = look("protoc-gen-go", "protoc-gen-go-grpc", "protoc-gen-go-http", "protoc-gen-go-errors", "protoc-gen-openapi"); err != nil {
		// update the xt plugins
		cmd := exec.Command("xt", "upgrade")
...
func generate(proto string, args []string) error {
	...
	inputExt := []string{
		"--proto_path=" + base.XTMod(),
		"--proto_path=" + filepath.Join(base.XTMod(), "third_party"),
